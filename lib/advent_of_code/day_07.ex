defmodule AdventOfCode.Day07 do
  ...
  def process_line("dir " <> dir, {current_dir,fs}), do: {current_dir,put_in(fs, Enum.reverse([dir | current_dir]), @node)}
  ...
  def update_total_size(fs, [_ | ancestors] = path, file_size) do
    get_and_update_in(fs, Enum.reverse([:total_size | path]), fn total_size -> {total_size, total_size + file_size} end)
  ...
  def part1(args), do: args |> parse_filesystem() |> (fn %{"/" => root} -> total_size_under_100000(root) end).()

  def part2(args) do
    %{"/" => %{total_size: used_space} = root} = parse_filesystem(args)
